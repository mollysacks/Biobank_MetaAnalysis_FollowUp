...
# 2. By chromosome PRS
#    After running provided script, put 22 output files into PRS_HEIGHT_by_chromosome
...
setwd('/path/to/Biobank_MetaAnalysis_FollowUp/Height') 
dir.create('output')

#### Read in CNV main effects
main_effects <- read.csv('CNVmainEffects_4cohorts_allCNVs_HEIGHT_20230531.csv')
...
my_genomatrix <- read.csv('/path/to/your_Height_genomatrix.csv')
...
genomatrix$res_height_inv <- my_genomatrix$res_height_inv # whatever your normalized HEIGHT column is called
...
# prs
genomatrix$PRS_HEIGHT_zscore <- my_genomatrix$HEIGHT_SCORE1_SUM_zscore # whatever your z-scored PRS_SCORE1_SUM column is called
...
prefix <- 'PRS_HEIGHT_by_chromosome/your_dataset_height_wood2014_pst_eff_a1_b0.5_phi1e-02.txt.chr'
suffix <- '.sscore'
...
for(i in 1:22){
  ...
  # add to genomatrix
  genomatrix[,paste('chr', as.character(i), 'PRS_HEIGHT', sep='')] <- PRS_i$SCORE1_SUM
}


# Additional QC (for UKBB, there were some PRS outliers, so we filter those out)
genomatrix <- genomatrix[which((genomatrix$PRS_HEIGHT_zscore > -3) & (genomatrix$PRS_HEIGHT_zscore < 3)),] # 
...
genomatrix$odd_chr_PRS_HEIGHT_sum <- 
  genomatrix$chr1PRS_HEIGHT +
  genomatrix$chr3PRS_HEIGHT +
  genomatrix$chr5PRS_HEIGHT +
  genomatrix$chr7PRS_HEIGHT +
  genomatrix$chr9PRS_HEIGHT +
  genomatrix$chr11PRS_HEIGHT +
  genomatrix$chr13PRS_HEIGHT +
  genomatrix$chr15PRS_HEIGHT +
  genomatrix$chr17PRS_HEIGHT +
  genomatrix$chr19PRS_HEIGHT +
  genomatrix$chr21PRS_HEIGHT 

genomatrix$odd_chr_PRS_HEIGHT_zscore <- (genomatrix$odd_chr_PRS_HEIGHT_sum - mean(genomatrix$odd_chr_PRS_HEIGHT_sum))/sd(genomatrix$odd_chr_PRS_HEIGHT_sum)

genomatrix$even_chr_PRS_HEIGHT_sum <- 
  genomatrix$chr2PRS_HEIGHT +
  genomatrix$chr4PRS_HEIGHT +
  genomatrix$chr6PRS_HEIGHT +
  genomatrix$chr8PRS_HEIGHT +
  genomatrix$chr10PRS_HEIGHT +
  genomatrix$chr12PRS_HEIGHT +
  genomatrix$chr14PRS_HEIGHT +
  genomatrix$chr16PRS_HEIGHT +
  genomatrix$chr18PRS_HEIGHT +
  genomatrix$chr20PRS_HEIGHT +
  genomatrix$chr22PRS_HEIGHT 

genomatrix$even_chr_PRS_HEIGHT_zscore <- (genomatrix$even_chr_PRS_HEIGHT_sum - mean(genomatrix$even_chr_PRS_HEIGHT_sum))/sd(genomatrix$even_chr_PRS_HEIGHT_sum)

### Create PRS squared column
genomatrix$PRS_HEIGHT_zscore_squared <- genomatrix$PRS_HEIGHT_zscore * genomatrix$PRS_HEIGHT_zscore


### Create PRS quantile columns
breaks <- .25
genomatrix$odd_chr_PRS_HEIGHT_quantile <- as.numeric(cut(genomatrix$odd_chr_PRS_HEIGHT_zscore, 
                                                      breaks = quantile(genomatrix$odd_chr_PRS_HEIGHT_zscore, 
                                                                        probs = seq(0, 1, by = breaks)), 
                                                      labels = FALSE))
genomatrix$even_chr_PRS_HEIGHT_quantile <- as.numeric(cut(genomatrix$even_chr_PRS_HEIGHT_zscore, 
                                                       breaks = quantile(genomatrix$even_chr_PRS_HEIGHT_zscore, 
                                                                         probs = seq(0, 1, by = breaks)), 
                                                       labels = FALSE))
genomatrix$PRS_HEIGHT_quantile <- as.numeric(cut(genomatrix$PRS_HEIGHT_zscore, 
                                                      breaks = quantile(genomatrix$PRS_HEIGHT_zscore, 
                                                                        probs = seq(0, 1, by = breaks)), 
                                                      labels = FALSE))
...
model_quadratic <- lm(res_height_inv ~ PRS_HEIGHT_zscore + PRS_HEIGHT_zscore_squared + age + sex + PC1 + PC2 + PC3 + PC4 + 
...
model_linear <- lm(res_height_inv ~ PRS_HEIGHT_zscore + age + sex + PC1 + PC2 + PC3 + PC4 + 
                        PC5 + PC6 + PC7 + PC8 + PC9 + PC10, 
                      data = genomatrix, na.action=na.exclude)
...
model <- lm(res_height_inv ~ odd_chr_PRS_HEIGHT_zscore*even_chr_PRS_HEIGHT_zscore + age + sex + PC1 + PC2 + PC3 + PC4 + 
                     PC5 + PC6 + PC7 + PC8 + PC9 + PC10, 
                   data = genomatrix, na.action=na.exclude)
...
model <- lm(res_height_inv ~ odd_chr_PRS_HEIGHT_zscore + age + sex + PC1 + PC2 + PC3 + PC4 + 
              PC5 + PC6 + PC7 + PC8 + PC9 + PC10, 
            data = genomatrix, na.action=na.exclude)
...
model <- lm(res_height_inv ~ even_chr_PRS_HEIGHT_zscore + age + sex + PC1 + PC2 + PC3 + PC4 + 
              PC5 + PC6 + PC7 + PC8 + PC9 + PC10, 
            data = genomatrix, na.action=na.exclude)
...
for(i in (1:4)){
  odd <- genomatrix[which(genomatrix$odd_chr_PRS_HEIGHT_quantile == i),]
  model_odd <- lm(res_height_inv ~ 
                    even_chr_PRS_HEIGHT_zscore +
                    age + sex + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10, 
                  data = odd, na.action=na.exclude)
  results <- summary(model_odd)$coefficients
  estimate <- as.numeric(results['even_chr_PRS_HEIGHT_zscore', 1])
  se <- as.numeric(results['even_chr_PRS_HEIGHT_zscore', 2])
  z <- as.numeric(results['even_chr_PRS_HEIGHT_zscore', 3])
  p <- as.numeric(results['even_chr_PRS_HEIGHT_zscore', 4])
  ci_lower <- estimate - (1.96*se)
  ci_upper <- estimate + (1.96*se)
  even_by_odd[nrow(even_by_odd) +1,] <- c(i, estimate, se, ci_lower, ci_upper, z, p)
  
  even <- genomatrix[which(genomatrix$even_chr_PRS_HEIGHT_quantile == i),]
  model_even <- lm(res_height_inv ~ 
                     odd_chr_PRS_HEIGHT_zscore +
                     age + sex + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10, 
                   data = even, na.action=na.exclude)
  results <- summary(model_even)$coefficients
  estimate <- as.numeric(results['odd_chr_PRS_HEIGHT_zscore', 1])
  se <- as.numeric(results['odd_chr_PRS_HEIGHT_zscore', 2])
  z <- as.numeric(results['odd_chr_PRS_HEIGHT_zscore', 3])
  p <- as.numeric(results['odd_chr_PRS_HEIGHT_zscore', 4])
  ci_lower <- estimate - (1.96*se)
  ci_upper <- estimate + (1.96*se)
...
model <- lm(res_height_inv ~ 
              PRS_HEIGHT_zscore*CNV_effect_continuous +
              age + sex + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10, 
            data = odd, na.action=na.exclude)
...
for(i in seq_along(keys)){
  ...
  estimate <- as.numeric(results['PRS_HEIGHT_zscore', 1])
  se <- as.numeric(results['PRS_HEIGHT_zscore', 2])
  z <- as.numeric(results['PRS_HEIGHT_zscore', 3])
  p <- as.numeric(results['PRS_HEIGHT_zscore', 4])
  ci_lower <- estimate - (1.96*se)
  ci_upper <- estimate + (1.96*se)
...
for(i in seq_along(genotypes)){
  ...
  model <- lm(res_height_inv ~ PRS_HEIGHT_zscore + age + sex + PC1 + PC2 + PC3 + PC4 + 
                PC5 + PC6 + PC7 + PC8 + PC9 + PC10, 
              data = df, na.action=na.exclude)
  res <- summary(model)$coefficients
  estimate <- as.numeric(res['PRS_HEIGHT_zscore', 1])
  se <- as.numeric(res['PRS_HEIGHT_zscore', 2])
  ci_lower <-estimate - (1.96*se)
  ci_upper <- estimate + (1.96*se)
  z <- as.numeric(res['PRS_HEIGHT_zscore', 3])
  p <- as.numeric(res['PRS_HEIGHT_zscore', 4])
...
for(i in seq_along(keys)){
  ...
  for(j in 1:4){
    df <- genomatrix[which((genomatrix$PRS_HEIGHT_quantile == j)),]
    if(nrow(df) == 0){
    ...
    model <- lm(res_height_inv ~ gen + age + sex + PC1 + PC2 + PC3 + PC4 + 
                  PC5 + PC6 + PC7 + PC8 + PC9 + PC10, 
                data = df, na.action=na.exclude)
    res <- summary(model)$coefficients
...
for(j in 1:4){
  df <- genomatrix[which(genomatrix$PRS_HEIGHT_quantile == j),]
  model <- lm(res_height_inv ~ group + age + sex + PC1 + PC2 + PC3 + PC4 + 
                PC5 + PC6 + PC7 + PC8 + PC9 + PC10, 
              data = df, na.action=na.exclude)
  res <- summary(model)$coefficients
...
for(i in seq_along(keys)){
  ...
  df$gen <- df[[geno]]
  df$gen <- relevel(as.factor(df$gen), ref="2")
  model <- lm(res_height_inv ~ gen*sex + age  + PC1 + PC2 + PC3 + PC4 + 
                PC5 + PC6 + PC7 + PC8 + PC9 + PC10, 
              data = df, na.action=na.exclude)
  res <- summary(model)$coefficients
...
model <- lm(res_height_inv ~ group*sex + age + PC1 + PC2 + PC3 + PC4 + 
                PC5 + PC6 + PC7 + PC8 + PC9 + PC10, 
              data = genomatrix, na.action=na.exclude)
res <- summary(model)$coefficients
...
for(i in seq_along(keys)){
  ...
  model_male <- lm(res_height_inv ~ gen + age + PC1 + PC2 + PC3 + PC4 + 
                PC5 + PC6 + PC7 + PC8 + PC9 + PC10, 
              data = genomatrix_male, na.action=na.exclude)
  ...
  model_female <- lm(res_height_inv ~ gen + age + PC1 + PC2 + PC3 + PC4 + 
                     PC5 + PC6 + PC7 + PC8 + PC9 + PC10, 
                   data = genomatrix_female, na.action=na.exclude)
...
model_male <- lm(res_height_inv ~ group + age + PC1 + PC2 + PC3 + PC4 + 
              PC5 + PC6 + PC7 + PC8 + PC9 + PC10, 
            data = genomatrix_male, na.action=na.exclude)
res <- summary(model_male)$coefficients
write.csv(res, 'output/15_CNVgroup_by_Sex/CNVgroup_by_Sex_male.csv')

model_female <- lm(res_height_inv ~ group + age + PC1 + PC2 + PC3 + PC4 + 
                   PC5 + PC6 + PC7 + PC8 + PC9 + PC10, 
                 data = genomatrix_female, na.action=na.exclude)
res <- summary(model_female)$coefficients
